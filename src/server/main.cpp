    ...
    try {
        while (true) {
            ...
            // 如果type為message，則處理訊息
                ...
                if (!insert_message(conn, username, message)) {
...
int main() {
    ...
    if (mysql_query(conn, "SHOW TABLES LIKE 'messages';")) {
        ...
        if (mysql_num_rows(res) == 0) {
            std::string create_message_query = "CREATE TABLE IF NOT EXISTS messages (id INT PRIMARY KEY AUTO_INCREMENT, username VARCHAR(255), message TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP);";
